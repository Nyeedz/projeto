...
class EditAvatarModal extends React.Component {
  ...
  handleSave = data => {
    ...
    // fetch(`${API_URL}uploadImg/${localStorage.getItem('idProfile')}`, {
    //   method: 'POST',
    //   headers: headerConfig.headers,
    //   body: this.dataURLtoBlob(img)
    // })
    //   .then(res => this.updateProfile())
    //   .catch(err => console.error(err));
  };

  updateProfile = () => {
    // let perfil = this.props.user;
    // perfil.linkPhoto = `${API_URL}downdImg/${localStorage.getItem(
    //   'idProfile'
    // )}?cache=${randomNumber()}`;
